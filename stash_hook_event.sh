...
function user_access() {
    ...
    curl -sS -u ${username}:${passwd} -X PUT "${qcs_repo}/${repo_name}/permissions/users?permission=${2}&name=${1}" -H "Accept: application/json, text/javascript, */*; q=0.01"
...
function cu_hooks_StashWebhookToJenkins() {
    ...
    curl -sS -u ${username}:${passwd} -X ${1} "${qcs_repo}/${repo_name}/${hook_key_ap}/enabled" -H "Content-Type: application/json"
    echo -e "\nop $repo_name done..\n"
}
...
function config_hooks_StashWebhookToJenkins() {
    ...
    curl -sS -u ${username}:${passwd} -X PUT "${qcs_repo}/${repo_name}/${hook_key_ap}/settings" -H "Content-Type: application/json" --data-binary "${payload_json}"
    echo -e "\nop $repo_name done..\n"
}

function config_hooks_code_events() {
    echo_ "configuring $repo_name Webhook with event to Jenkins"
    curl -sS -u ${username}:${passwd} -X POST "${qcs_repo}/${repo_name}/webhook/create/1?url=${new_rule}" -H "Content-Type: application/json"
    echo -e "\nop $repo_name done..\n"
}
